...
def nJyToMagnitude(flux):
    ...
    # See Oke & Gunn (1983), unlabeled equation on page 3.
    # The reference flux is ~3631, the commonly quoted "AB magnitude zero point".
    referenceFlux = 1e23 * 10**(48.6/-2.5)
    return -2.5*np.log10(flux*1e-9/referenceFlux)
...
class PhotoCalibTestCase(lsst.utils.tests.TestCase):
    ...
    def _testPhotoCalibCenter(self, photoCalib, calibrationErr):
        ...
        errFlux = computeMaggiesErr(self.instFluxErr,
                                    self.instFlux,
                                    calibrationErr,
                                    self.calibration,
                                    self.flux)
        ...
        errFlux2 = computeMaggiesErr(self.instFluxErr,
                                     self.instFlux2,
                                     calibrationErr,
                                     self.calibration,
                                     self.flux2)
